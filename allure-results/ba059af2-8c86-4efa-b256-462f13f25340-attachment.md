...
- Name: API_test_suite.spec.js >> Autenticação e Perfis >> Login com Credenciais Válidas (Admin)
- Location: tests\API_test_suite.spec.js:215:6
...
```ts
  ...
  217 |   
  218 |   let responsePOSTlogin_Admin = await page.request.post('http://localhost:3000/login',
  219 |       {
  220 |         data: {
...
> 234 |     expect(bodyPOSTlogin_Admin.usuario.senha).not.toHaveProperty('senha');
      |                                                   ^ Error: expect(received).not.toHaveProperty(path)
  235 |     expect(bodyPOSTlogin_Admin.usuario.tipo).toHaveProperty('tipo');
