...
Expected: "Usuário admin principal não pode ser deletado"
Received: "Admin principal não pode ser deletado"
```
...
```ts
  ...
  136 | 
> 137 |         expect(body3DELETEuser.mensagem).toBe('Usuário admin principal não pode ser deletado');
      |                                          ^ Error: expect(received).toBe(expected) // Object.is equality
  138 | 
  139 |     });
  140 | })
