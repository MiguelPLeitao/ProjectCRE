...
Received value: "{\"id\":1,\"nome\":\"Admin Master\",\"email\":\"admin@biblioteca.com\",\"tipo\":3}"
```
...
> 170 |         expect(usuario).toBeInstanceOf(Object);
      |                         ^ Error: expect(received).toBeInstanceOf(expected)
  171 |         expect(JSON.parse(usuario)).toHaveProperty('tipo');
  172 |         expect(JSON.parse(usuario).tipo).toBe(3);
  173 | 
  174 | 
  175 |         /* const usuario = await page.evaluate(() => {
  176 |              const raw = localStorage.getItem('usuario');
  177 |              try { return raw ? JSON.parse(raw) : null; }
  178 |              catch { return null; }
  179 |          });
  180 |  
  181 |          expect(usuario).toBeTruthy();      // existe e é um objeto
  182 |          expect(usuario.tipo).toBe(3);     // valida tipo === 3
  183 |  */
  184 |     });
  185 | })
```