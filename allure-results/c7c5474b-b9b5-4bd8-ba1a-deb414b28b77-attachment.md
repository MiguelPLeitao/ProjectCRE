...
```ts
  ...
  66 | 
  67 |         const {finalTitle} =await books_page.Fill_AllNewBook_Fields();
  68 | 
  ...
  76 |         await expect(books_page.EstoqueLivro_BookStock_spinbutton).toHaveValue("1");
  77 |         await expect(books_page.PrecoLivro_BookPrice_spinbutton).toHaveValue("0");
  78 | 
