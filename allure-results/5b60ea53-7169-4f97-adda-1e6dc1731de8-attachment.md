...
Locator: locator('[id="lista-arrendamentos"] .book-card').filter({ hasText: 'In Cold Blood' }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('[id="lista-arrendamentos"] .book-card').filter({ hasText: 'In Cold Blood' }).first()

```

```yaml
- heading "📅 Meus Arrendamentos" [level=1]
- text: Courtney HillsALUNO
- button "Sair"
- link "Dashboard":
  - /url: dashboard.html
- link "Livros":
...
- 'heading "Arrendamento #2" [level=3]'
- paragraph:
  - strong: "Livro ID:"
  - text: "13"
- paragraph:
