...
```ts
  ...
  51 | console.log(User2arrayPosition.tipo);
  52 |         let newType;
  53 |         if (User2arrayPosition.tipo === 2) {
  54 |             newType = 3;
  55 |         }
  56 |         else if (User2arrayPosition.tipo === 3) {
  57 |             newType = 2;
  58 |         }
  59 |         else {
  60 |             console.log("Tipo de usuário: " + User2arrayPosition.tipo);
> 61 |             throw new Error("Tipo de usuário inválido ou não pode ser alterado");
     |                   ^ Error: Tipo de usuário inválido ou não pode ser alterado
  62 |         }
  63 | 
  64 |         let response2PUTuser = await page.request.put(`/usuarios/${randomUser}`,
  65 |             {
  66 |                 data: {
  67 |                     nome: User2arrayPosition.nome + " - Atualizado",
  68 |                     email: "atualizado" + User2arrayPosition.email,
  69 |                     tipo: newType
  70 |                 }
  71 |             }
  72 |         );
  73 | 
  74 |         expect(response2PUTuser.status()).toBe(200);
  75 | 
  76 |         let body2PUTuser = await response2PUTuser.json();
  77 | 
  78 |         expect(body2PUTuser).toHaveProperty('id');
  79 |         expect(body2PUTuser.id).toBe(randomUser);
  80 |         expect(body2PUTuser).toHaveProperty('nome');
  81 |         expect(body2PUTuser.nome).toBe(User2arrayPosition.nome + " - Atualizado");
  82 |         expect(body2PUTuser).toHaveProperty('email');
  83 |         expect(body2PUTuser.email).toBe("atualizado" + User2arrayPosition.email);
  84 |         expect(body2PUTuser).toHaveProperty('tipo');
  85 |         expect(body2PUTuser.tipo).toBe(newType);
  86 |         expect(body2PUTuser).not.toHaveProperty('senha');
  87 |     });
  88 | })
```