...
- generic [ref=e2]:
  - generic [ref=e3]:
    ...
    - generic [ref=e5]:
      - generic [ref=e6]: Peggy UllrichALUNO
...
> 95  |         page.waitForEvent('dialog').then(async dialog => {
  ...
  106 | 
  107 |         await rents_page.Select_Book_dropdown(newBook_random.nome);
  108 |         await rents_page.Select_StartDate_datepicker(null);
  109 |         await rents_page.Select_EndDate_datepicker(null);
  110 |         await rents_page.ClickSolicitarArrendamento_RequestRent_button();
  111 | 
  112 |         await expect(page).toHaveURL('http://localhost:3000/arrendamentos.html');
  113 | 
  114 |         const CreatedRentCard = await rents_page.SelectArrendamento_RentCard_grid(newBook_random.nome);
  115 |         await expect(CreatedRentCard).toBeVisible();
  116 | 
  117 |     });
  118 | })
```